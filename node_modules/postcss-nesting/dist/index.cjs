  ...
  return toSelectors.map(toSelector => {
    return parser(selectors => {
      selectors.walkNesting(selector => {
        ...
        if (selector.next() && selector.next().type === 'tag') {
        ...
        if (selector.prev() && selector.prev().type !== 'combinator' && fromSelectorAST.first && fromSelectorAST.first.first && fromSelectorAST.first.first.type === 'tag') {
          if (complexFromSelector) {
            selector.replaceWith(fromSelectorWithIsAST.clone());
            return;
          }
...
function transformNestRuleWithinRule(node, walk) {
  // move previous siblings and the node to before the parent
...
function atruleWithinRule(node, walk) {
  // move previous siblings and the node to before the parent
  const parent = shiftNodesBeforeParent(node); // clone the parent as a new rule with children appended to it
