...
const customPropertyRegExp$1 = /^--[A-z][\w-]*$/; // whether the node is an html or :root rule
...
const isCustomDecl = node => node.type === 'decl' && customPropertyRegExp$1.test(node.prop); // whether the node is a parent without children
...
var transformProperties = ((decl, customProperties, opts) => {
  if (isTransformableDecl(decl) && !isRuleIgnored(decl)) {
    const originalValue = decl.value;
    const valueAST = postcssValuesParser.parse(originalValue);
    let value = String(transformValueAST(valueAST, customProperties)); // protect against circular references

    const valueSet = new Set();

    while (customPropertiesRegExp.test(value) && !valueSet.has(value)) {
      valueSet.add(value);
      const parsedValueAST = postcssValuesParser.parse(valueAST);
...
const customPropertyRegExp = /^--[A-z][\w-]*$/; // match custom property inclusions

const customPropertiesRegExp = /(^|[^\w-])var\([\W\w]+\)/; // whether the declaration should be potentially transformed

const isTransformableDecl = decl => !customPropertyRegExp.test(decl.prop) && customPropertiesRegExp.test(decl.value); // whether the declaration has a trailing comment
...
const creator = opts => {
  ...
  let customProperties; // whether to return synchronous function if no asynchronous operations are requested
  ...
  return {
    ...
    prepare({
      ...
      if (canReturnSyncFunction) {
        ...
        return {
          Declaration: decl => transformProperties(decl, customProperties, {
            preserve
          })
        };
      } else {
        return {
          Once: async root => {
            customProperties = Object.assign({}, getCustomPropertiesFromRoot(root, {
              preserve
